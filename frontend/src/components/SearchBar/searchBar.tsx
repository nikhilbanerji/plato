...
interface SearchBarProps {
    setRecipes: (recipes: Array<{ id: number; title: string; image?: string; ingredients?: string }>) => void;
}
...
const SearchBar: React.FC<SearchBarProps> = ({ setRecipes }) => {
    ...
    async function fetchRecipes(searchTerm: string) {
        try {
            const response = await fetch(
                `http://localhost:8080/plato/recipes/complexSearch?query=${encodeURIComponent(searchTerm)}&number=10`
            );
            if (!response.ok) {
                throw new Error(`Server responded with ${response.status}`);
            }
